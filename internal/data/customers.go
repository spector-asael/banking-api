...
func (m CustomerModel) Insert(customer *Customer) error {
	query := `INSERT INTO customers (person_id, kyc_status_id, created_at, updated_at)
			  VALUES ($1, $2, NOW(), NOW()) RETURNING id, created_at, updated_at`
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	err := m.DB.QueryRowContext(ctx, query, customer.PersonID, customer.KYCStatusID).Scan(
		&customer.ID, &customer.CreatedAt, &customer.UpdatedAt,
	)
	if err != nil {
		// Check for unique violation by error string (works for SQLite, Postgres, etc.)
		if err.Error() != "" && (err.Error() == "UNIQUE constraint failed: customers.person_id" || err.Error() == "pq: duplicate key value violates unique constraint \"unique_person_customer\"") {
...
func (m CustomerModel) GetByID(id int64) (*Customer, error) {
	query := `SELECT id, person_id, kyc_status_id, created_at, updated_at FROM customers WHERE id = $1`
	var c Customer
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	err := m.DB.QueryRowContext(ctx, query, id).Scan(
		&c.ID, &c.PersonID, &c.KYCStatusID, &c.CreatedAt, &c.UpdatedAt,
	)
	if err != nil {
...
func (m CustomerModel) GetAll(filters Filters) ([]*Customer, Metadata, error) {
	query := `SELECT count(*) OVER(), id, person_id, kyc_status_id, created_at, updated_at
			  FROM customers
	...
	for rows.Next() {
		var c Customer
		if err := rows.Scan(&totalRecords, &c.ID, &c.PersonID, &c.KYCStatusID, &c.CreatedAt, &c.UpdatedAt); err != nil {
			return nil, Metadata{}, err
		}
